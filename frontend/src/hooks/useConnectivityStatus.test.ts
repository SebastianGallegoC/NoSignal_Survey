...
describe("useConnectivityStatus helpers", () => {
  it("buildHealthUrl usa la base del API cuando existe", () => {
    expect(buildHealthUrl("https://api.percens.site")).toBe(
      "https://api.percens.site/health",
    );
  });
  ...
  it("checkConnectivity devuelve true cuando /health responde ok", async () => {
    ...
    await expect(
      checkConnectivity("https://api.percens.site/health", fetchImpl as typeof fetch),
    ).resolves.toBe(true);
